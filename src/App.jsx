...
const initialTasks = [
  {
    ...
    due: "02 Nov",
    hours: "2h",
    note: "Salón campestre confirmado.",
  },
  {
    id: 2,
    ...
    due: "03 Nov",
    hours: "1h",
    note: "Invitaciones enviadas a 120 invitados.",
  },
  {
    id: 3,
    ...
    hours: "2h",
    urgent: true,
    note: "Vence hoy: decisión de menú para evitar penalizaciones contractuales.",
  },
  {
    id: 4,
    ...
    due: "05 Nov",
    hours: "5h",
    note: "Pendiente comparar tres cotizaciones.",
  },
];
  ...
  return (
    <div className="overlay">
      <section
        ...
        aria-label={title}
      >
        <button className="close" aria-label="Cerrar" onClick={close}>
          ×
        </button>
        <h2>{title}</h2>
        {children}
...
export default function App() {
  ...
  const save = (status) => {
    setTasks((all) =>
      all.map((t) =>
        t.id === active.id ? { ...t, status, note: note || t.note } : t,
      ),
    );
    ...
    notify(
      status === "Hecho"
        ...
        : "Gestión pospuesta correctamente",
    );
  };
  return (
    <main className="app">
      <header>
        ...
        <nav aria-label="Navegación principal">
          <button
            className={view === "eventos" ? "selected" : ""}
            onClick={() => setView("eventos")}
          >
            Eventos
          </button>
          <button
            className={view === "hoy" ? "selected" : ""}
            onClick={() => setView("hoy")}
          >
            Hoy
          </button>
        ...
        <button
          ...
          onClick={() => notify("Evento creado correctamente")}
        >
          + Crear evento
        </button>
      </header>
      ...
      {view === "eventos" ? (
        <section className="page">
          ...
          <article className="event">
            ...
            <div>
              ...
              <p>
                ▣ 18 Nov 2025{" "}
                <span>
                  ◷ Límite de gestión: <b>4 horas/día</b>
                </span>
              </p>
            </div>
            <button
              ...
              onClick={() => notify("El evento se envió a la papelera")}
            >
              ⌫
            </button>
          </article>
          <section className="card progress">
            <div>
              ...
              <p>
                {done} de 4 tareas completadas ({4 - done} pendientes)
              </p>
            </div>
            <strong>{done * 25}%</strong>
            <div className="track">
              <span style={{ width: `${done * 25}%` }} />
            </div>
          </section>
          <section className="card tasks">
            ...
            {tasks.map((task) => (
              <article className="task" key={task.id}>
                <i className={`check ${task.status.toLowerCase()}`}>
                  {icons[task.status]}
                </i>
        ...
        <Today
          ...
          active={active}
          setActive={setActive}
          open={() => setModal("execute")}
        />
      )}
      {modal === "execute" && (
        <Modal title="Registrar ejecución" close={() => setModal("")}>
          <p>
            Actualiza el estado de <b>{active.name}</b>.
          </p>
          ...
          <textarea
            ...
            placeholder="Escribe un detalle para el historial..."
          />
          <div className="actions">
            <button className="btn ghost" onClick={() => save("Pospuesto")}>
              ↻ Posponer
            </button>
            <button className="btn primary" onClick={() => save("Hecho")}>
              ✓ Marcar como hecho
            </button>
          </div>
        </Modal>
        ...
        <Modal title="Editar evento" close={() => setModal("")}>
          ...
          <div className="actions">
            <button
              ...
              onClick={() => {
                setModal("");
                notify("Cambios del evento guardados");
              }}
            >
              Guardar cambios
            </button>
      ...
      {modal === "reschedule" && (
        <Modal title="Reprogramar tarea" close={() => setModal("")}>
          ...
          <p>
            Esta tarea requiere <b>5 horas estimadas</b>.
          </p>
          ...
          <div className="conflict">
            ...
            <p>
              Las tareas programadas para este día superan el límite diario
              definido.
            </p>
          ...
          <div className="suggestion">
            ✓{" "}
            <span>
              <b>Fecha sugerida: 07 de Noviembre</b>
              <br />
              0h ocupadas de 4h disponibles
            </span>
          </div>
          <div className="actions">
            <button className="btn ghost" onClick={() => setModal("")}>
            ...
            <button
              ...
              onClick={() => {
                setModal("");
                notify("Tarea reprogramada para el 07 de noviembre");
              }}
            >
...
function Today({ tasks, active, setActive, open }) {
  const groups = [
    [
      "Gestiones urgentes",
      "Atención inmediata",
      tasks.filter((t) => t.urgent && t.status !== "Hecho"),
    ],
    [
      "Otras gestiones pendientes",
      "Programadas para hoy",
      tasks.filter((t) => !t.urgent && t.status === "Pendiente"),
    ],
    [
    ...
    <section className="page">
      ...
      <div className="today">
          ...
          {groups.map(([title, subtitle, group]) => (
            <section className="card group" key={title}>
              ...
              {group.length ? (
                group.map((task) => (
                  <button
                    className={`today-task ${task.urgent ? "urgent" : ""}`}
                    key={task.id}
        ...
        <aside className="card panel">
          ...
          <h2>{active.name}</h2>
          <p>
            ◷ {active.hours} estimadas &nbsp; ▣ {active.due}
          </p>
          ...
          <small>
            Puedes marcarla como hecha o posponerla con una nota opcional.
          </small>
        </aside>
      </div>
    </section>
